...
describe('clientsController', () => {
	...
	it('#createOne should create one client', async () => {
		...
		td.when(clientModel.createOne(req.body, hashClientResult)).thenResolve({ client: 'client-created' });
	...
	it('#updateClient should update one client identified by its id', async () => {
		const clientModel = td.replace('../../../src/models/ClientModel');
		td.when(clientModel.getOne('some-client-id')).thenResolve({ id: 'some-client-id' });
		...
		const hash = clientModel.hashClient(JSON.stringify(req.body));
		td.when(clientModel.updateClient(req.body, hash, req.params)).thenResolve();

		const updateClientResult = await ClientsController.updateClient(req);

		expect(updateClientResult)
			.to.be.an('object')
			.and.has.property('message')
			.that.is.an('string')
			.that.equals('success');
